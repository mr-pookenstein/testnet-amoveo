...
function channels_main() {
    ...
    function main_view() {
        ...
        var refresh_channels_button = button_maker2("Server mode", function() {
            variable_public_get(["pubkey"], function(pubkey) {
                return refresh_channels_interfaces(pubkey);
            });
        });

        refresh_channels_button.setAttribute("id", "refresh_button");

        var refresh_channels2 = button_maker2("Peer to peer mode", function() {
            //you have to say your peer's pubkey in order to load your channel with them.
            return refresh_channels_peer_interfaces();
        });

        refresh_channels2.setAttribute("id", "refresh_button2");
        ...
        channel_sync_button2.setAttribute("id", "sync_button");



        append_children(channels_div, [channel_warning_div, load_button, br(), channel_sync_button2, br(), br(), save_name, save_button, br(), refresh_channels_button, refresh_channels2, br(), br()]);
...
//        div.appendChild(channel_sync_button);
	...
	bets_div.id = "bets_div";
        ...
        if (read(pubkey) == undefined) {
            ...
            append_children(div, [amount_info, spend_amount, br(), height_button, cost_button]);
        } else {
